 ...
 */
 ...
 $(document).ready(function(){
 	...
 	$("#audioFileForm").on("submit", function(){
 		...
 		let formData = new FormData($('#audioFileForm')[0]);
 		//업로드된 파일명 알아오기 : imageBox에 이미지 출력하기위해
 		//c:\fakepath\cat.mp3 경로및파일명 : c: | fakepath | cat.mp3 =>pop() => cat.mp3
 		...
 		$.ajax({
 			...
 			success:function(result){
 				if(result=="success"){
 					// cat.mp3를 업로드 했다면
 					//<audio src="/audio/cat.jpg" ...> 가 div 태그내에 삽입
 					//브라우저가 해석하면서 /images/cat.jpg 이미지 파일 서버에게 요청
 					console.log('<audio src="/audio/' + fileName +'" controls>');
